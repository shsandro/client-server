...
bool init_server(server *self, const char *host_path)
{
    ...
    if (read_hostfile(host_path, &self->socket_address) == false)
        return false;
...
bool init_client(client *self)
{
    ...
    if (read_hostfile("./hosts/hostfile_proxy", &self->socket_address) == false)
        return false;
...
bool init_proxy(proxy *self)
{
    ...
    if (self->socket == 0)
    {
        perror("Não foi possível criar o socket PROXY.\n");
        return false;
    }
    ...
    printf("Socket PROXY criado.\nSocket descriptor: %d.\n", self->socket);

    if (read_hostfile("./hosts/hostfile_server1", &self->socket_server_address[VIDEO_SERVER]) == false)
        return false;
    if (read_hostfile("./hosts/hostfile_server2", &self->socket_server_address[MUSIC_SERVER]) == false)
        return false;
    if (read_hostfile("./hosts/hostfile_server3", &self->socket_server_address[PHOTO_SERVER]) == false)
        return false;

    if (bind(self->socket, (struct sockaddr *)&self->socket_address, self->sockaddr_lenght) < 0)
    {
        perror("Bind PROXY falhou.\n");
        return false;
    }

    if (listen(self->socket, 5) < 0)
    {
        perror("Listen PROXY falhou.\n");
        return false;
    }
